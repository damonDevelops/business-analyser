import streamlit as st

def main():
    st.title("🤝 Contribute to Business Analyzer")

    st.markdown("""
    ## Contributing

    **Business Analyzer** is an open-source project, and we welcome contributions from the community! Whether you're interested in fixing bugs, adding new features, or improving documentation, your help is greatly appreciated.

    ### How to Contribute

    1. **Fork the Repository** on [GitHub](https://github.com/damonDevelops/business-analyser).

    2. **Clone Your Fork** to your local machine.

       ```bash
       git clone https://github.com/damonDevelops/business-analyser.git
       cd business-analyzer
       ```

    3. **Create a New Branch** for your feature or bug fix.

       ```bash
       git checkout -b feature/YourFeatureName
       ```

    4. **Make Your Changes** and commit them.

       ```bash
       git commit -m "Add some feature"
       ```

    5. **Push to Your Fork:**

       ```bash
       git push origin feature/YourFeatureName
       ```

    6. **Open a Pull Request** on the main repository.

    ### Reporting Issues

    If you find any bugs or have suggestions for improvements, please [open an issue](https://github.com/damonDevelops/business-analyser) on GitHub.

    ### License

    This project is licensed under the [MIT License](https://github.com/damonDevelops/business-analyser/blob/master/LICENSE).

    ### Acknowledgements

    - Developed by [@DamonDevelops](https://damon-develops.tech)
    - [Streamlit](https://streamlit.io/)
    - [Google Places API](https://developers.google.com/places/web-service/overview)
    - [TextBlob](https://textblob.readthedocs.io/en/dev/)
    """)

if __name__ == "__main__":
    main()
